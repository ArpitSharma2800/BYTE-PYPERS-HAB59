...
fetch("https://byte-pypers.herokuapp.com/list_items_get", requestOptions)
    ...
    .then(result => {
        ...
        for (var i = 0; i < listingResult.prod.length; i++) {
            ...
            var id = listingResult.prod[i]._id;
            console.log(name)
            console.log(price)
            console.log(quantity)


            var listingContent = `<tbody><tr id="list${i+1}"><th scope="row">${i+1}</th><td>${name}</td><td>${quantity}</td><td>${price}</td><td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">Drop Price</button></td><td><button type="button" class="btn btn-danger" id="${id}" onClick="clickHandler(id)">Delete Item</button></td></tr></tbody>`
